...
def create_inception_vars(in_num, reduce1x1, mapn):
    ...
    W1x3_1 = create_weight([1, 3, reduce1x1, reduce1x1])
    W3x1_2 = create_weight([3, 1, reduce1x1, reduce1x1])
    W1x3_3 = create_weight([1, 3, reduce1x1, reduce1x1])
    W3x1_4 = create_weight([3, 1, reduce1x1, mapn])
    B1x1_2 = tf.Variable(tf.zeros([reduce1x1]))
    B1x1_3 = tf.Variable(tf.zeros([reduce1x1]))
    B1 = tf.Variable(tf.zeros([mapn*4]))
    lst = (W1x1_1, W1x1_2, W1x1_3, W1x1_4, W1x3, W3x1, W1x3_1, W3x1_2, W1x3_3, W3x1_4, B1, B1x1_2, B1x1_3)
    return lst


def model_inception(data, *args):
    (W1x1_1, W1x1_2, W1x1_3, W1x1_4, W1x3, W3x1, W1x3_1, W3x1_2, W1x3_3, W3x1_4, B1, B1x1_2, B1x1_3) = args
    conv1 = conv2d_s(data, W1x1_1, s=2)
    conv2 = tf.nn.relu(conv2d_s(data, W1x1_2) + B1x1_2)
    conv3x3 = conv2d_s(conv2d_s(conv2, W1x3), W3x1, s=2)
    conv3 = tf.nn.relu(conv2d_s(data, W1x1_3) + B1x1_3)
    conv5x5 = conv2d_s(conv2d_s(conv2d_s(conv2d_s(conv3, W1x3_1), W3x1_2), W1x3_3), W3x1_4, s=2)
    conv4 = conv2d_s(max_pool_3x3_s(data, s=2), W1x1_4)
    incept = tf.nn.relu(tf.concat([conv1, conv3x3, conv5x5, conv4], axis=3) + B1)
    print incept.shape
    return incept
